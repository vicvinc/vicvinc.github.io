...
excerpt: 软键盘调起导致的fixed视窗滚动，和viewport产生滚动差，导致点击区域错位
---

在移动端的 dialog 或者 popup 中提交表单或者其他需要调用软键盘的输入操作的时候，软键盘的弹起会让视窗（viewport）向上滚动，在结束输入软键盘消失之后并没有把视窗滚回到原来的位置，但是渲染视图还是跟之前一样，这个时候如果点击之前的按钮或者输入框的时候，就会产生错位。
